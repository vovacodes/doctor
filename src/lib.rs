...
use nom::error::{context, convert_error, make_error, ErrorKind, VerboseError};
...
pub struct InlineTag<'a> {
    pub name: &'a str,
    pub body_lines: Option<Vec<&'a str>>,
}
...
fn doc_comment<'a>() -> impl Parser<&'a str, DocComment<'a>, VerboseError<&'a str>> {
    context(
        "doc_comment",
        all_consuming(tuple((
            comment_start(),
